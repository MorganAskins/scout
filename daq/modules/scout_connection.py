...
class sis_connection():
    ...
    def __init__(self, interface, host):
        ...
        # Confirm the connection is robust

    def connect(self, iname, host):
        self.ifconfig('%s down' % iname)
        self.ifconfig('%s %s' % (iname, host))
        self.ifconfig('%s up' % iname)
        self.dhcpd(2, quiet=True)
        timeout = 10 # timeout if no connection is made
        count = 0
        while self.sis_ip is None:
            count += 1
            self.sis_address()
            time.sleep(1)
            if count > timeout:
                print 'Connection to sis3316 refused. Is it on?'
                break
        return self.sis_ip
    ...
    def sis_address(self):
        ...
        for k,v in pairs.iteritems():
            if 'sis3316' in k:
              ...
              address = v
        if total_address > 1:
            # The dhcpd leases file has become weird ... reset time
            self.connect()
            return None
        if address:
            try: 
                output = subprocess.check_output('ping -c 1 %s' % address, 
                        shell=True)
                self.sis_ip = address
                return address
            except Exception, e:
